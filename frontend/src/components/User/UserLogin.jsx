...
import { Login, getCurrentUser } from "../../services/user.service.js";
...
const UserLogin = () => {
  ...
  const InputOnChange=(property,value)=>{
    ...
    const handleSubmit = (e) => {
      ...
      //navigate('/ReadUser')
      /*
      const session = Login(formObj).then(
          res=>{
            if(res==true){
              setUser(formObj);
              navigate('/ReadUser');
      ...
      Axios.post("http://localhost:5050/api/v1/user/Login", {
        email,
        password,
      }).then(response => {
